...
for i in range(1, n_sales + 1):
    ...
    sales_rows.append({
        "date": d, "region": rng.choice(regions), "product_category": cat,
        "product_name": prod, "quantity": qty, "unit_price": price,
        "total_revenue": total, "cost": cost, "profit": round(total - cost, 2),
...
for i in range(1, 201):
    ...
    cust_rows.append({
        "name": f"Customer_{i}", "email": f"customer{i}@example.com",
        "region": rng.choice(regions), "signup_date": sd,
        "lifetime_value": round(float(rng.uniform(500000, 50000000)), 2), "orders_count": int(rng.integers(1, 100)),
...
for i, d in enumerate(dates[:180], 1):
    ...
    wa_rows.append({
        "date": str(d.date()), "page_views": pv, "unique_visitors": uv,
