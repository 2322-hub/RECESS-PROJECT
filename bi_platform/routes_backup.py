...
def _json_response(data, status=200):
    return Response(
        json.dumps(_json_safe(data), default=safe_json_serialize),
        status=status,
...
if _db_url and not _db_url.startswith("sqlite"):
    ...
    try:
        db_connector.connect("countrysales", _db_url)
    except Exception as _e:
        logger.warning("Could not auto-connect countrysales: %s", _e)
...
_CONN_TABLE_MAP: dict[str, str] = {
    "countrysales": "country_sales",
}


def _get_table_for_conn(conn: str) -> str:
    return _CONN_TABLE_MAP.get(conn, "sales")
...
_COLUMN_MAP = {
    ...
    "profit": ("profit", "net_profit", "gross_profit", "earnings", "total_profit"),
    "quantity": ("quantity", "qty", "units", "units_sold", "count"),
    "region": ("region", "area", "territory", "location", "state", "city"),
    "product_category": ("product_category", "category", "product_type", "type", "item_type"),
    "product_name": ("product_name", "product", "name", "item_name", "product_name"),
    "customer_segment": ("customer_segment", "segment", "customer_type", "client_type", "sales_channel"),
}
...
def _normalize_columns(df: pd.DataFrame, table_type: str = "sales") -> pd.DataFrame:
    ...
    if table_type == "sales":
        ...
        # Normalise date to YYYY-MM-DD regardless of source format
...
def _normalize_dashboard_filters(filters: dict | None) -> dict:
    ...
    for key, value in (filters or {}).items():
        normalized_key = _FILTER_ALIASES.get(key, key)
        if normalized_key in {"region", "product_category", "customer_segment"} and value:
            normalized[normalized_key] = value
    return normalized


def _apply_column_filter(df: pd.DataFrame, column: str, value) -> pd.DataFrame:
    if df.empty or column not in df.columns or not value:
        return df
    series = df[column].astype(str)
    if isinstance(value, list):
        choices = [str(item) for item in value if item not in (None, "")]
        return df[series.isin(choices)] if choices else df
...
def _apply_date_filter(df: pd.DataFrame, candidates: tuple[str, ...], start: str | None, end: str | None) -> pd.DataFrame:
    ...
    if start:
        mask &= dates >= pd.to_datetime(start, errors="coerce")
    if end:
        mask &= dates <= pd.to_datetime(end, errors="coerce")
    return df[mask]
...
def _filter_dashboard_data(
    ...
    sales = _apply_column_filter(sales, "region", filters.get("region"))
    sales = _apply_column_filter(sales, "product_category", filters.get("product_category"))
    sales = _apply_column_filter(sales, "customer_segment", filters.get("customer_segment"))
    sales = _apply_date_filter(sales, ("date",), date_start, date_end)

    customers = _apply_column_filter(customers, "region", filters.get("region"))
    customers = _apply_column_filter(customers, "segment", filters.get("customer_segment"))
    customers = _apply_date_filter(customers, ("date", "signup_date", "created_at"), date_start, date_end)

    website = _apply_date_filter(website, ("date", "created_at"), date_start, date_end)
    ...
    if conn == "demo":
        ...
        try:
            ...
            with engine.connect() as c:
                count = c.execute(text(f"SELECT COUNT(*) FROM {table}")).scalar() or 0  # noqa: S608
...
def api_table_data(table_name: str):
    ...
    try:
        ...
        logger.error("Explorer data error for '%s.%s': %s", conn, table_name, e, exc_info=True)
        return jsonify({"error": f"Could not load table '{table_name}'"}), 400
...
def api_filter():
    data = request.get_json(force=True)
    table = data.get("table", _get_table_for_conn(data.get("conn", "demo")))
    ...
    try:
        sales, customers, website = _load_dashboard_data(conn)
        sales, customers, website = _filter_dashboard_data(sales, customers, website, filters, date_start, date_end)
        payload = analytics.build_dashboard_payload(sales, customers, website)
        payload["_conn"] = conn
        payload["_mode"] = "filtered"
        payload["_filters"] = _normalize_dashboard_filters(filters)
        payload["row_count"] = len(sales)
        return _json_response(payload)
    except Exception as e:
        logger.error("Filter error for '%s': %s", conn, e, exc_info=True)
        return jsonify({"error": str(e), "conn": conn}), 400
...
def api_anomalies():
    ...
    try:
        if _use_sql_mode(conn):
            table = _get_table_for_conn(conn)
            monthly = db_connector.sql_monthly_trends(conn, table=table)
        else:
            sales, _, _ = _load_dashboard_data(conn)
            monthly = analytics.monthly_trends(sales)
...
def api_generate_report(fmt: str):
    ...
    if not dashboard_data:
        try:
            if _use_sql_mode(conn):
                table = _get_table_for_conn(conn)
                dashboard_data = analytics.build_dashboard_payload_sql(db_connector, conn, table=table)
            else:
                sales, customers, website = _load_dashboard_data(conn)
